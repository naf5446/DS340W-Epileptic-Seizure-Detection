      ...
      if (is.na(out$total.seizures)) { 
        ...
        for (i in 1:nrow(out$seizures)) {
          ...
          for (j in 1:18) {
            ...
            # bug fix on 15.04.2023
            # According to:
            # https://cran.r-project.org/doc/manuals/r-devel/NEWS.html
            # Setting digits = 0 in format(), print.default() (and hence typically print()) or options() 
            # is again invalid. Its behaviour was platform-dependent, and it is unclear what “zero significant digits” should mean (PR#18098). 
            #
            # In the below 'format' function the 'digits' was set to 1 instead of 0
            ticks <- format(
              seq(ss[1, 19] + 1, 
              format(ss[nrow(ss), 19] + 1, digits = 1), by = 2), 
              digits = 1)        
            if (j == 1) {
